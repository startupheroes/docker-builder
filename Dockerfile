...
RUN apt-get update && \
  apt-get install -y nano libxml2-utils build-essential git curl tar gzip libjpeg-dev imagemagick ttf-dejavu jpegoptim grep libpng-dev --no-install-recommends && \
  mkdir -p ~/.m2
